    ...
    for episode in range(num_loops):
        ...
        for step in range(sep.max_steps):
            ...
            if dlp.smcp_mode == 'topk':
                weight_init, idx = torch.topk(par_weight, dlp.num_par_smc_init)
            ...
            weight_init = torch.softmax(weight_init, -1).unsqueeze(1).repeat(1, dlp.num_par_smc)  # [M, N]
            states_init = par_states[idx]  # [K, C] -> [M, C]
            states_init_ = np.reshape(states_init, (1, dlp.num_par_smc_init, 1, sep.dim_state))  # [1, M, 1, C]
            smc_states = np.tile(states_init_, (dlp.horizon, 1, dlp.num_par_smc, 1))  # [1, M, 1, C] -> [T, M, N, C]
            smc_action = np.zeros((dlp.horizon, dlp.num_par_smc, sep.dim_action))  # [T, N, dim_a]
            smc_weight = torch.log(torch.ones((dlp.num_par_smc)).to(dlp.device) * (1.0 / float(dlp.num_par_smc)))  # [N]
            mean_state = np.reshape(mean_state, (1, 1, sep.dim_state))  # [1, 1, C]
            smc_mean_state = np.tile(mean_state, (dlp.horizon, dlp.num_par_smc, 1))  # [T, N, C]
            prev_q = 0

            for i in range(dlp.horizon):
                curr_smc_state = torch.FloatTensor(smc_states[i]).to(dlp.device)  # [M, N, C]
                action, log_prob = model.policy.get_action(
                    torch.FloatTensor(smc_mean_state[i]).to(dlp.device),  # [N, C]
                    torch.transpose(curr_smc_state, 0, 1).contiguous().view(dlp.num_par_smc, -1))  # [N, M * C]
                action_tile = action.unsqueeze(0).repeat(dlp.num_par_smc_init, 1, 1).view(-1, sep.dim_action)

                next_smc_state = model.dynamic_net.t_model(
                    torch.FloatTensor(smc_states[i]).to(dlp.device).view(-1, sep.dim_state), action_tile * sep.step_range)
                ...
                mean_par = model.dynamic_net.t_model(
                    torch.FloatTensor(smc_mean_state[i]).to(dlp.device), action * sep.step_range)
                ...
                q = model.get_q(curr_smc_state.view(-1, sep.dim_state), action_tile).view(dlp.num_par_smc_init, -1)
                advantage = q - prev_q - log_prob.unsqueeze(0).repeat(dlp.num_par_smc_init, 1)  # [M, N]
            ...
            if step % dlp.pf_resample_step == 0:
                if dlp.pp_exist:
                    ...
                    proposal_state = model.pp_net(torch.FloatTensor(curr_obs).unsqueeze(0).to(dlp.device), num_par_propose)
